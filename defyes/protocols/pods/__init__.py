...
class BaseVault(BaseVault):
    ...
    def timestamp(self) -> float:
        return self._chain_explorer.time_from_block(self.block)

    @cached_property
    def utc(self):
        return utc_from_timestamp(self.timestamp)

    @cached_property
    def utc_year_beginning(self):
        return utc(self.utc.year, 1, 1)

    @cached_property
    def utc_month_beginning(self):
        return utc(self.utc.year, self.utc.month, 1)

    @cached_property
    def at_30days_before(self) -> BaseVault:
        t30days = 30 * 24 * 3600
        prev_timestamp = self.timestamp - t30days
        prev_block = self._chain_explorer.block_after(prev_timestamp)
        return self.__class__(self.blockchain, prev_block, self.address)

    @cached_property
    def at_year_beginning(self) -> BaseVault:
        prev_block = self._chain_explorer.block_after(self.utc_year_beginning.timestamp())
        return self.__class__(self.blockchain, prev_block, self.address)

    @cached_property
    def at_month_beginning(self) -> BaseVault:
        prev_block = self._chain_explorer.block_after(self.utc_month_beginning.timestamp())
        return self.__class__(self.blockchain, prev_block, self.address)

    @cached_property
    def at_previous_month_beginning(self) -> BaseVault:
        prev_month_somewhere = self.at_month_beginning.utc - timedelta(days=28)
        prev_month_beginning = utc(prev_month_somewhere.year, prev_month_somewhere.month, 1)
        prev_block = self._chain_explorer.block_after(prev_month_beginning.timestamp())
        return self.__class__(self.blockchain, prev_block, self.address)
...
class Period:
    ...
    def no__repr__(self):
        return f"(rate={self.rate.percent} period='{self.period_timedelta}' APY={self.apy.percent})"
